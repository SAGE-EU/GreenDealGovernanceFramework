...
For product lifecycle, circular economy, Digital Product Passport, logistics, and traceability scenarios, EPCIS and CBV are relevant options for interoperable visibility event data where GS1 identifiers are used.
...
In product, asset, location, and supply-chain contexts, exchange interfaces may also include GS1 Digital Link resolver services and EPCIS repository interfaces, as the domain counterparts to the geospatial interfaces noted above.
...
API descriptions should identify the semantic artefacts needed for interpretation, including identifier schemes, event vocabularies, link types, and validation artefacts. Where GS1 standards are used, references should point to the relevant GS1 Digital Link, EPCIS or CBV, and GS1 Web Vocabulary artefacts; where other standards are used, references point to their equivalents.
