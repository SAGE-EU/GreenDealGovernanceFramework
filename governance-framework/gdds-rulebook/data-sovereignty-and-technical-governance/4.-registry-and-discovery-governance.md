...
_Source: D3.1 GDDS first version report (WP3), Section 3 (SAGE Core components: Participant Registry, Authorisation Registry, Catalogue)._ &#x20;
...
_Note (to confirm, or discuss with WP3 for implementation): Governance should provide selective disclosure at discovery: the catalogue may expose an offering and its metadata to a broad audience while access to the underlying data and to any sensitive attributes remains policy-controlled, so that confidential use cases can be represented in the catalogue without revealing protected detail. This is realised through the discovery filtering in subsection 1.6 and, technically, in the Technical Framework (see Publication and Discovery); the D6.2 use-case inputs (for example UC6.1, UC8, and UC10) identified it as a requirement._
{% endhint %}
